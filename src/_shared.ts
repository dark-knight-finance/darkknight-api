...
import { PAIR_RESERVES_BY_TOKENS, PAIRS_VOLUME_QUERY_STRING, SWAPS_BY_PAIR, TOP_PAIRS, PAIR_FROM_TOKENS, TOTAL_LIQUIDITY, BUNDLE_PRICE_ETH, TOKEN_BY_ADDRESS } from './apollo/queries'
...
export async function getBundleEthPrice(): Promise<[any]> {
  let {data} = await client.query({
    query: BUNDLE_PRICE_ETH
  })
  return data
}
